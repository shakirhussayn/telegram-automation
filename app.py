    ...
    if event.message.text and event.message.text.startswith('/set'):
        async with lock:
            ...
            if len(parts) == 2:
                ...
                updated = False
                if key == "STAFF_NAME":
                    STAFF_NAME = new_value.strip(); updated = True
                elif key == "DATE":
                    DATE = new_value.strip(); updated = True
                elif key == "PHOTO_LOCATION":
                    PHOTO_LOCATION = new_value.strip(); updated = True
                elif key == "START_DAILY_NUM":
                    daily_counter = int(new_value.strip()); updated = True
                elif key == "START_HISTORY_NUM":
                    history_counter = int(new_value.strip()); updated = True
